    ...
    def writeCppPrototype(w: IndentWriter) {
      ...
      w.w("struct " + actualSelf + cppFinal).bracedSemi {
        ...
        if(r.fields.nonEmpty) {
          w.wl
          writeAlignedCall(w, actualSelf + "(", r.fields, ")", f => marshal.fieldType(f.ty) + " new_" + idCpp.local(f.ident))
          w.wl
          val init = (f: Field) => idCpp.field(f.ident) + "(std::move(new_" + idCpp.local(f.ident) + "))"
